...
// =========================================
const float KP_WALL = 9.60;       // แรงดึงกลับเข้ากลาง (ถ้าวิ่งชนกำแพงให้เพิ่ม, ถ้าส่ายให้ลด)
const float KD_WALL = 16.75;      // แรงหน่วงการส่าย (ช่วยหยุดการแกว่ง) <-- เพิ่มตัวนี้
const int WALL_CENTER_DIST = 42.5; // ระยะห่างจากกำแพงที่ต้องการ (mm)
const int WALL_VALID_MAX = 55;  
const int MAX_WALL_ADJUST = 60;
...
// ... existing variables ...
const int CELL_DISTANCE_TICKS = 1035; 
...
// PID Constants
float Kp_turn = 11.00; float Ki_turn = 0.01; float Kd_turn = 1.0;
...
void initDistSensors() {
  ...
  for (int i = 0; i < NUM_SENSORS; i++) {
    ...
    // --- จุดที่แก้ไข ---
    sensors[i].init();            // เรียกเฉยๆ ไม่ต้องใส่ if
    sensors[i].configureDefault(); // แนะนำให้เพิ่มบรรทัดนี้เพื่อตั้งค่ามาตรฐาน
    sensors[i].setAddress(0x30 + i);
    sensors[i].setTimeout(500);
    
    // แก้ชื่อฟังก์ชัน startContinuous เป็น startRangeContinuous
    // ใส่เลข 50 คือให้อ่านค่าทุกๆ 50ms (ปรับได้)
...
void moveForwardSync(int targetSpeed) {
  // 1. อ่านค่าระยะ
  int distL = getDistanceMM(3); // ซ้าย
  int distR = getDistanceMM(2); // ขวา
...
bool wallFront() {  
    ...
    return (getDistanceMM(0) < 88 && getDistanceMM(1) < 82); 
} 
