...
// Gyro Turn
float Kp_turn = 9.75, Ki_turn = 0.015, Kd_turn = 0.975;//---------------------------------------------------------------------------------------------
float GYRO_SCALE = (90.0/63.125); // ค่าแก้สเกล Gyro จากโค้ดเก่า
...
void brakeMotors() {
  // สั่ง HIGH (255) ทั้งสองขาเพื่อให้เกิด Short Brake (ล้อล็อก)
  // ใช้ analogWrite 255 เพื่อความชัวร์ว่า PWM ถูกสั่งเต็ม Max
...
void forceBrake() {
  // 1. กระชากกลับหลัง (Reverse Kick) เป็นเวลาสั้นๆ
  // ต้องระวัง! ถ้าค่าน้อยไปจะหยุดไม่อยู่ ถ้ามากไปรถจะถอยหลัง
  setSpeed(-255, -255); 
  delay(100); // <-- ลองปรับค่านี้ดู (50 ถึง 150 ms)
  
  // 2. ล็อกล้อ (Short Brake)
  brakeMotors();
  delay(200); // พักให้รถนิ่งสนิทจริงๆ
}
// =========================================
...
// วิ่งตรง (Sync Motors)
// ฟังก์ชันวิ่งตรง พร้อมระบบ Soft Start ในตัว
// ฟังก์ชันวิ่งตรง พร้อมระบบ Soft Start ในตัว
void moveForwardSync(int targetSpeed) {
  // --- ส่วนที่ 1: คำนวณ Soft Start ---
  unsigned long elapsed = millis() - rampStartTime;
  int currentSpeed = targetSpeed; // ตั้งต้นที่ความเร็วเป้าหมาย

  // ถ้ายังไม่ครบเวลา RAMP_DURATION ให้ลดความเร็วลงตามสัดส่วน
  if (elapsed < RAMP_DURATION) {
    currentSpeed = map(elapsed, 0, RAMP_DURATION, MIN_PWM, targetSpeed);
  }

  // --- ส่วนที่ 2: รักษาสมดุลล้อ (Sync Logic) ---
  long error = countL - countR;
  int adjustment = (int)(error * KP_sync);

  // --- ส่วนที่ 3: สั่งมอเตอร์ ---
  // ใช้ currentSpeed ที่คำนวณมาแล้ว + adjustment
...
// ฟังก์ชันวิ่งตรงตามระยะทางที่กำหนด (Blocking)
void driveDistance(long targetTicks) {
  // 1. รีเซ็ตค่าเริ่มต้น
  countL = 0;
  countR = 0;
  rampStartTime = millis(); // เริ่มนับเวลา Soft Start ใหม่
  
  Serial.printf("🚗 Drive Distance: %ld ticks\n", targetTicks);

  // 2. ลูปวิ่งจนกว่าจะถึงระยะ
  while (abs(countL) < targetTicks) { // ใช้ abs เผื่อวิ่งถอยหลังในอนาคต
    
    updateYaw(); // *สำคัญ* ต้องเรียกเพื่อให้ Gyro ไม่เพี้ยน
    ...
    // --- สั่งมอเตอร์ ---
    moveForwardSync(currentSpeed); // เรียกฟังก์ชันที่คุณมีอยู่แล้ว
  }

  // 3. ถึงเป้าหมายแล้วหยุด
  brakeMotors();
...
//     // ============================================
//     // 🔥 เขียน Sequence การเดินตรงนี้ 🔥
//     // ============================================
    
//     // Step 1: เดินหน้า 20,000 ticks
//     driveDistance(1000); 
//     delay(500); // พักนิดนึงเพื่อความนิ่ง

//     // Step 2: เลี้ยวซ้าย 90 องศา
//     turnPID(-90.0); 
//     delay(500);

//     // Step 3: เดินหน้าต่ออีก 20,000 ticks
//     driveDistance(500);
...
// }
// void loop() {
//   // 1. อ่านค่า Gyro ตลอดเวลา (สำคัญมาก ห้ามเอาออก)
//   updateYaw();
...
//     countL = 0; 
//     countR = 0; 
//     currentYaw = 0; // *สำคัญ* เซ็ตทิศปัจจุบันให้เป็น 0
    
//     Serial.println("Starting Turn Test...");
//     delay(1000); // รอ 1 วินาที ให้มือเราออกจากหุ่นก่อน
    
//     running = true; // อนุญาตให้เข้าเงื่อนไขข้างล่าง
//   }
...
// }
void loop() {
  if (digitalRead(START_BUTTON) == HIGH) { // สมมติปุ่มกดติดปล่อยดับ เป็น HIGH
    delay(1000);
    alignFront(); // เรียกใช้ตรงนี้
    brakeMotors();
